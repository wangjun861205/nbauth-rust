...
fn main() {
    ...
    let expire_duration: ExpireDuration =
        Duration::days(jwt_days.parse().expect("非法的jwt有效时长"));
    let retry_limit: RetryLimit = env::var("RETRY_LIMIT")
        .expect("读取重试次数限制失败")
        .parse()
        .expect("非法的重试次数限制");
    let retry_interval = RetryInterval(Duration::minutes(
        env::var("RETRY_INTERVAL")
            .expect("读取重试间隔失败")
            .parse()
            .expect("非法的重试间隔"),
    ));
